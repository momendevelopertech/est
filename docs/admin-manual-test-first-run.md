...
- لو أنشأت جلسة جديدة بنفسك، بعد الحفظ افتح تفاصيل الجلسة أولاً ثم ادخل `Assignments`.
- اختيار المبنى داخل الجلسة يحدد المباني المسموح بالتوزيع داخلها لهذه الجلسة.
- `Deactivate` ليس `Delete permanently`: التعطيل يخفي السجل من الشغل النشط، أما الحذف النهائي فيزيله من قاعدة البيانات عندما لا توجد سجلات مرتبطة.

## 4.1 افهم التوزيع الآلي والتحقق قبل القفل قبل ما تبدأ

### معنى `تحقق ما قبل القفل`

- هذا فحص جاهزية يتم قبل تحويل الجلسة إلى `LOCKED`.
- إذا كانت قائمة `تحقق ما قبل القفل` تحتوي على مشاكل، فلن يسمح لك السيستم بالقفل.
- السطر العربي الظاهر لك هو التسمية المترجمة للمشكلة، والسطر الإنجليزي هو الرسالة الأصلية القادمة من الباك.

### معنى رسالة `room_capacity_min_not_satisfied`

- هذه الرسالة تعني أن القاعة لم تصل بعد للحد الأدنى المطلوب من التغطية.
- `المطلوب` = قيمة `capacityMin` المضبوطة على القاعة.
- `الحالي` = عدد التوزيعات النشطة الموجودة فعليًا داخل نفس القاعة على مستوى `ROOM`.
- إذا ظهرت الرسالة مكررة عدة مرات فهذا يعني أن أكثر من قاعة ما زالت ناقصة، وليس أن هناك خطأ واحدًا فقط.

### لماذا قد ترى `المطلوب: 4 / الحالي: 1` بعد تشغيل Auto Assign؟

- لأن التوزيع الآلي الحالي لا يوزع حسب `capacityMin` مباشرة.
- اللوجيك الحالي يبني slot واحد لكل `role` على كل قاعة.
- لذلك لو عندك role واحد فقط للقاعة مثل `room_proctor`، فالنتيجة الطبيعية قد تكون `1` لكل قاعة حتى لو القاعة نفسها تحتاج `3` أو `4`.
- في هذه الحالة زر `تشغيل التوزيع الآلي` وحده لن يكون كافيًا للقفل، وستحتاج إما تكملة يدوية أو تعديل `capacityMin` على القاعات التجريبية.

### معنى أزرار التوزيع

- `محاكاة توزيع آلي`: تشغيل نفس منطق التوزيع الآلي بدون حفظ أي assignments.
- `تشغيل التوزيع الآلي`: إنشاء auto assignments فعلية على الجلسة.
- `محاكاة إعادة الترتيب`: preview لما سيحدث لو أعدت ترتيب الـ auto draft الحالية بدون حفظ.
- `تشغيل إعادة الترتيب`: تنفيذ إعادة الترتيب فعليًا مع إعادة بناء الـ `AUTO DRAFT` القابلة لذلك.
- إعادة الترتيب لا تعني "حل أي نقص" تلقائيًا؛ هي فقط تعيد حساب توزيع الـ auto draft الحالية مع الحفاظ على الـ manual assignments والـ auto غير الـ draft.

### عمليًا أثناء التست

- في أول مرور استخدم `محاكاة توزيع آلي` و`محاكاة إعادة الترتيب` أولًا.
- لو ظهر نقص تغطية، لا تعتبره crash؛ هذا business validation طبيعي.
- لو هدفك فقط إكمال التست والوصول إلى `LOCKED` بسرعة، فإما تضيف room assignments يدويًا أو تقلل `capacityMin` على القاعات التجريبية حتى يصبح `الحالي >= المطلوب`.
