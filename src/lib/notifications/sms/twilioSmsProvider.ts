import type {
  SmsProviderRuntimeConfigContract,
  SmsProviderSendInputContract,
  SmsProviderSendResultContract
} from "./contracts";
...
function parseTwilioCredentials(config: SmsProviderRuntimeConfigContract) {
...
export function createTwilioSmsProvider(): SmsProviderAdapter {
  return {
    key: "twilio",
    async sendMessage(
      ...
      if (!credentials) {
        return {
          ...
          response: null,
          error:
            "Twilio credentials are missing. Configure sms_account_sid plus sms_api_key, or sms_api_key as accountSid:authToken."
        };
      }
      ...
      try {
        ...
        return {
          ...
          response: null,
          error:
            error instanceof Error ? error.message : "Unexpected Twilio provider error."
        };
      }
    }
  };
