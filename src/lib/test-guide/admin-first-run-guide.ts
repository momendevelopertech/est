...
export const adminFirstRunGuide: ManualTestGuide = {
  ...
  cards: [
    ...
    {
      title: "فهم التوزيع والقفل",
      items: [
        "`تحقق ما قبل القفل` هو فحص جاهزية الجلسة قبل تحويلها إلى `LOCKED`.",
        "إذا ظهر نفس التحذير أكثر من مرة فغالبًا كل سطر يخص قاعة مختلفة ما زالت ناقصة تغطية.",
        "`المطلوب / الحالي` يعني الحد الأدنى المطلوب للقاعة مقابل عدد التوزيعات النشطة الموجودة الآن.",
        "`محاكاة` تشغّل نفس المنطق بدون حفظ، بينما `تشغيل` يحفظ فعليًا على البيانات.",
        "`محاكاة توزيع آلي` = preview بدون حفظ، و`تشغيل التوزيع الآلي` = إنشاء auto assignments فعلية.",
        "`محاكاة إعادة الترتيب` = preview لإعادة توزيع الـ auto draft الحالية، و`تشغيل إعادة الترتيب` = تنفيذها فعليًا.",
        "رسالة `room_capacity_min_not_satisfied` تعني أن عدد التوزيعات داخل القاعة أقل من `capacityMin` المضبوط عليها.",
        "التوزيع الآلي الحالي لا يضمن وحده الوصول إلى `capacityMin` لكل قاعة؛ أحيانًا تحتاج تكملة يدوية."
      ]
    }
  ],
  sections: [
    ...
    },
    {
      ...
      notes: [
        ...
        "لو أنشأت جلسة جديدة بنفسك بدل استخدام seed، فبعد الحفظ افتح تفاصيل الجلسة أولاً ثم ادخل `Assignments`. اختيار المبنى هنا يعني أن التكليفات المسموح بها لهذه الجلسة ستكون داخل هذا المبنى أو هذه المباني فقط.",
        "داخل القوائم: `تعطيل` يوقف السجل ويخفيه من الشغل النشط مع الاحتفاظ به للمراجعة، أما `حذف نهائي` فيزيله من قاعدة البيانات نهائيًا ولا ينجح إلا إذا لم تعد هناك سجلات مرتبطة."
      ]
    },
    {
      id: "assignments",
